...
gridoxi=griddata(lon,lat,oxi,xi,yi,'cubic');
...
txt=['OXIGENIO(ml/l) - ' num2str(prof_hor) 'm'];
...
name=['oxigenio_' num2str(prof_hor) 'm.png'];
print ('-dpng',name);

%%Plotagem colorida
disp(['VALORES MÍNIMO E MÁXIMO DE OXIGENIO'])
disp([min(oxi) max(oxi)])
barmin=input(['Qual o valor mínimo de oxigenio ' '\n' ...
    'a ser plotado na BARRA DE CORES?? == ']);
barmax=input(['Qual o valor máximo de oxigenio ' '\n' ...
    'a ser plotado na BARRA DE CORES?? == ']);
lim=[barmin barmax];
pcolorc(gridoxi,x,y,lim);
grid on; hold on; shading interp;

%%plotagem de isolinhas de oxigenio
inc=input('Qual o valor do intervalo entre as isolinhas de oxigenio?? === ');
oxi=[barmin:inc:barmax];
c=extcontour(xi,yi,gridoxi,oxi,'label',100,'fontsize',8,'color',[0.1 0.1 0.1]);
...
text(-47.95,-23.95,'Santos','fontsize',7,'color',[1 1 1]);
plot(-46.4,-23.95,'w.');%%%%%%