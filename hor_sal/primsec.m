% ROTINA PARA COMPARAÇAO ENTRE OS PERFIS PRIMÁRIOS E SECUNDÁRIOS
...
% MODIFICAR A LINHA 28 DA ROTINA COM O NÚMERO DE LETRAS DO NOME DO ARQUIVO
...
%%%% DESCIDA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Modificar os limites de T e S dos eixos (axis) usados nas estaçoes
%%%% rasas e profundas
subplot(131) %%TEMPERATURA
    ...
    ylabel('\bfProfundidade (m)');
         ...
         if min(profd) >= -200; %estaçoes rasas
...
subplot(132)  %SALINIDADE
    ...
    xlabel('\bfSal');
         ...
         if min(profd) >= -200; %estaçoes rasass
...
% Insere titulo central com numero da estacao
    ...
    txt=['\bfESTAÇAO ' named(j,1) named(j,5:6) ' - red(prim) blue(sec)']; title(txt); % MODIFICAR COLUNAS
    handle = get(gca,'title');
    set(handle,'visible','on');
    namest=['estação_' named(j,5:6) '_down.png']; % MODIFICAR COLUNAS
    print ('-dpng',namest);
%     
%%%% SUBIDA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Modificar os limites de T e S dos eixos (axis) usados nas estaçoes 
...
subplot(131)  %TEMPERATURA
    ...
    ylabel('\bfProfundidade (m)');
         ...
         if min(profs) >= -200; %estaçoes rasas
...
% Insere titulo central com numero da estacao
    axes('position',[.1 .88 .8 .05],'Box','off','Visible','off');
    txt=['\bfESTAÇAO ' names(j,1) names(j,5:6) ' - red(prim) blue(sec)']; title(txt); % MODIFICAR COLUNAS
    handle = get(gca,'title');
    set(handle,'visible','on');
    namest=['estação_' names(j,5:6) '_up.png']; % MODIFICAR COLUNAS
    print ('-dpng',namest);
end
